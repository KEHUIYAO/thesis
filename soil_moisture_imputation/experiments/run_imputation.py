import sys
sys.path.append('/home/kehuiyao/thesis/soil_moisture_imputation/')
sys.path.append('/Users/kehuiyao/Desktop/thesis/soil_moisture_imputation')
...
from data import GaussianProcess, DescriptiveST, DynamicST, SoilMoistureSparse, SoilMoistureHB, HealingMnist, Sine, Cluster, Kaust
...
def parse_args():
    ...
    # parser.add_argument("--dataset-name", type=str, default='kaust_point')
    ...
    # parser.add_argument("--config", type=str, default='imputation/st_transformer_air36.yaml')
...
def get_dataset(dataset_name: str):
    ...
    if dataset_name == 'gp':
        return add_missing_values(GaussianProcess(num_nodes=36, seq_len=4000), p_fault=p_fault, p_noise=p_noise, min_seq=12, max_seq=12 * 4, seed=56789)

    if dataset_name == 'descriptive':
        return add_missing_values(DescriptiveST(num_nodes=36, seq_len=4000), p_fault=p_fault, p_noise=p_noise, min_seq=12, max_seq=12 * 4, seed=56789)
    ...
    if dataset_name == 'kaust':
        return add_missing_values(Kaust(), p_fault=p_fault, p_noise=p_noise,
                                  min_seq=12, max_seq=12 * 4, seed=56789)
