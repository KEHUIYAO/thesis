...
class BRITS(nn.Module):
    ...
    def __init__(self, input_size: int, n_nodes: int, hidden_size: int = 64, u_size: int = 0):
        super().__init__()
        self.n_nodes = n_nodes
        self.rits_fwd = RITS(input_size * n_nodes, hidden_size, u_size)
        self.rits_bwd = RITS(input_size * n_nodes, hidden_size, u_size)

    def forward(self, x, mask=None, u=None, **kwargs):
        # tsl shape to original shape: [b s n c] -> [b s c]
        x = rearrange(x, 'b s n c -> b s (n c)')
        if u is not None:
            u = rearrange(u, 'b s n c -> b s (n c)')
        mask = rearrange(mask, 'b s n c -> b s (n c)')
        # forward
        imp_fwd, pred_fwd = self.rits_fwd(x, mask, u=u)
        # backward
        x_bwd = reverse_tensor(x, dim=1)
        if u is not None:
            u_bwd = reverse_tensor(u, dim=1)
        else:
            u_bwd = None
        mask_bwd = reverse_tensor(mask, dim=1) if mask is not None else None
        imp_bwd, pred_bwd = self.rits_bwd(x_bwd, mask_bwd, u=u_bwd)
